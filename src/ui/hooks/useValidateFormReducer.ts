    ...
    switch (action.type) {
      case 'first':
        return vName(value) === 'valid'
          ...
          : { ...state, first: { ...state.first, value, valid: 'invalid', error: 'Please include a valid first name.' } }
      case 'last':
        return vName(value) === 'valid'
      ...
      case 'gender':
        return vGender(value) === 'valid'
          ...
          : { ...state, gender: { ...state.gender, value, valid: 'invalid', error: 'Please include a gender or type N/A.' } }
      case 'age':
        return vAge(value) === 'valid'
          ? { ...state, age: { ...state.age, value, valid: valid ? valid : 'valid', error: null } }
          : { ...state, age: { ...state.age, value, valid: 'invalid', error: 'Please include a valid age between 10 - 120.' } }
      case 'email':
        return vEmail(value) === 'valid'
          ? { ...state, email: { ...state.email, value, valid: valid ? valid : 'valid', error: null } }
          : { ...state, email: { ...state.email, value, valid: 'invalid', error: 'Please include a valid email address.' } }
      case 'phone':
        return vPhone(value) === 'valid'
          ? { ...state, phone: { ...state.phone, value, valid: valid ? valid : 'valid', error: null } }
          : { ...state, phone: { ...state.phone, value, valid: 'invalid', error: 'Please include a valid Canadian phone number.' } }
      case 'vote':
      ...
      case 'referendum':
        return vReferendum(value) === 'valid'
          ...
          : { ...state, referendum: { ...state.referendum, value, valid: 'invalid', error: 'Please include a response to the referendum.' } } 
      case 'already-voted': 
